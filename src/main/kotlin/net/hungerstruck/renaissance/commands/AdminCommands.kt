...
object AdminCommands {
    ...
    fun startMatch(args: CommandContext, sender: CommandSender) {
        ...
        val playerLobby = player.rplayer.lobby

        if (playerLobby?.nextMap == null) {
            player.sendPrefixMessage("Next map has not been set. Use /setnext", ChatColor.RED)
            return
        }

        if (Renaissance.countdownManager.hasCountdown(RLobbyEndCountdown::class.java) and !force) {
            throw CommandException("Countdown has already started. Use the '-f' flag to force start.")
        }

        if(force) {
            player.sendPrefixMessage("Force starting the game.", ChatColor.RED)
            Renaissance.countdownManager.cancelAll()
            player.rplayer.lobby!!.startMatch()
        } else {
    ...
    fun stopLobbyCountdown(args: CommandContext, sender: CommandSender) {
        ...
        if(!player.rplayer.lobby!!.counting) {
            player.rplayer.lobby!!.counting = true
        }
        Renaissance.countdownManager.cancelAll()
