...
class BloodModule(match: RMatch, modCtx: RModuleContext) : RModule(match, modCtx) {
    ...
    fun onPlayerHit(event: EntityDamageEvent) {
        if (event.isCancelled || !isMatch(event.entity)) {
            return
        }

        match.players.filter {
            it.getSetting<Boolean>(Settings.BLOOD_OPTIONS) == true }
                .forEach{ it.bukkit.spawnParticle(particle.particle, it.bukkit.location, particle.count) }
    }
}
