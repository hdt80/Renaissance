...
class TNTSettingsModule(match: RMatch, modCtx: RModuleContext) : RModule(match, modCtx) {
    ...
    fun onEntityExplodeEvent(event: EntityExplodeEvent) {
        if (!isMatch(event.location.world)) return
            ...
            for (i in 0..15) {
                for (j in 0..15) {
                    if (k == 0 || k == 15 || i == 0 || i == 15 || j == 0 || j == 15) {
                        ...
                        while (f > 0.0f) {
                            val block = event.location.world.getBlockAt(Math.floor(d4).toInt(), Math.floor(d5).toInt(), Math.floor(d6).toInt())
                            if (block.type != Material.AIR && block.type != Material.WATER && block.type != Material.STATIONARY_WATER ) {
                                val f2 = 1 // relies on Strukkit durability api patch
                                f -= (f2 + 0.3f) * 0.3f
                            }
